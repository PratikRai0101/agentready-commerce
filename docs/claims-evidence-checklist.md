...
| 2 | Ambiguous “black shoes under ₹5,000” triggers clarification for size + use only (the two hard blockers), not purchase | `packages/catalog/src/ranking.ts` (`REQUIRED_HARD_CONSTRAINTS`, `missingHardConstraints`), `apps/web/lib/dialogue.ts` (`nextClarification`), `apps/web/lib/services.ts` (`respond`) | Supported |
| 3 | Ranking is grounded: Max Cushion ₹4,899 best overall / Streak 4 ₹4,299 cheaper / Stride Lite ₹3,499 trade-off, catalog-verbatim with compromises; fit scores on the public host are mock | `packages/catalog/src/catalog.ts` prices; `packages/catalog/src/ranking.ts`; `docs/evidence/llm-verification-3msg.md` shortlist table; mock spend note in `apps/web/lib/services.ts` (`composeShortlistMessage`, “MOCK settlement”) | Supported |
...
| 6b | Checkout rail selector: “Choose payment method” offers Razorpay Checkout (mock) and Agent Pay with x402 (Devnet simulation); selection binds to the approved digest; service layer blocks the losing rail both ways | `POST /api/pay/x402-order/prepare` + `/confirm`; `prepareX402OrderPayment`/`confirmX402OrderPayment` + cross-rail guards in `verifyPayment`/`mockCapture`/`initiatePayment`/`compensate`; `apps/web/app/page.tsx` modal; `apps/web/test/rail-selector.test.ts` (9 tests) | Supported |
...
| 8 | Razorpay Test Mode lifecycle verified: 3 checkouts, 2 authenticated webhooks, 1 Test Mode refund, `PAID_VERIFIED` before fulfilment (integration-path evidence, not live money) | `docs/evidence/razorpay-test-proof.md`: `order_TWTuHSmXrkHoUJ`/`pay_TWU2Fy64pOAaZi` ₹3848; `order_TWVIgwsRyjV7C8`/`pay_TWVJ9xLsjtdwoo` webhook `TWVJJZ01UBcNy1`; `order_TWVLQtCV7OXCmI`/`pay_TWVLknN4NRrHSN` webhook `TWVLtSP9a4RfZ4` + Razorpay Test Mode refund `rfnd_TWVNeD4HStaNby` processed → `REFUNDED`; 2026-08-31/09-01 | Supported (recorded evidence) |
| 9 | x402 settled on Devnet (0.01 USDC, v2 exact, memo-bound) and is not a second shoe charge; the live request returned HTTP 500 and was reconciled read-only, so on-screen scores are mock | `docs/devnet-settlement-evidence-app-path-2026-09-04.md`: sig `5FQb8Jh7LTmwoecXpv7TGDos61oFqo66T74uYY6mA6cWuD2EaHTQ73FsY2EZ99Wsj7j3SknsT4WE8vDmGxtt1Vfk`, slot `493082743`, finalized, `meta.err` null, single `accept` → HTTP 500 → `operator-cli reconcile-settled` with zero resubmissions; mint `4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU`, payer `4aCDfCtWrrVA74n8z8XDeKSHq2ddD95E68SHno5Y4Ap5`, payee `FF6Uj3ff6tp9smbCqfvJyBTrmef8ketsm3x8v1QPTrZ1`, memo `agentcart:v1:6826a0b9…`; explorer `…/tx/5FQb8Jh7…?cluster=devnet` | Supported (recorded 2026-09-04) |
...
| 11 | Graceful failure: price/variant change after approval → `REAPPROVAL_REQUIRED`, original digest invalidated, stale approval + payment blocked with named fields (self-contained, one request) | `POST /api/demo/price-drift` (approve → tamper → stale retries in one request); `apps/web/app/demo/page.tsx` “Price change after approval” / “Variant change”; `apps/web/test/demo-scenarios.test.ts` | Supported |
| 12 | Duplicate/replay safety: webhook dedup by event ID — first processed fresh, second deduplicated, one transition (self-contained, one request) | `POST /api/demo/webhook-replay` (one session to PAYMENT_PENDING, same webhook twice under one event ID); `apps/web/lib/webhook.ts` (`x-razorpay-event-id` dedup); conformance `gate_10`, `gate_15`; `apps/web/test/razorpay-binding.test.ts`, `apps/web/test/demo-scenarios.test.ts` | Supported |
